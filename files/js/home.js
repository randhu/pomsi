...
(function ($, Drupal) {
  ...
  Drupal.behaviors.editorialBootstrap = {
    attach: function (context, settings) {
      if (context !== document) return;

      this.headerScroll();
      this.clickableTeasers();
      this.clickableTables();
      this.openMobileParent();
      this.affixBlogSidebar();
    },
    ...
    clickableTeasers: function () {
      $('.article-teaser').on('click', function () {
        var target = $(this).find('h4 a').attr('href');
        if (target !== undefined) {
          window.location.href = target;
        }
      });

      $('.most-read-article').on('click', function () {
      ...
      $('.aggregation-item').on('click', function () {
        var target = $(this).find('h4 a').attr('href');
        if (target !== undefined) {
          window.location.href = target;
        }
    ...
    clickableTables: function () {
      $('table.clickable-table tr').on('click', function () {
        var target = $(this).find('a').attr('href');
        if (target !== undefined) {
          window.location.href = target;
        }
    ...
    openMobileParent: function () {
      $(".mm-listview .nav-item.dropdown .dropdown-toggle").on('click', function () {
        window.location.href=$(this).attr("href");
      });
    },

    affixBlogSidebar: function () {
      if ($(".path-race .d-md-block .nav").length) {
        var $blogMenu = $(".path-race .d-md-block .nav");
        var offset = $blogMenu.offset().top;
        var trigger = 0; //point where affix must start
        
        if ($("body.scrolled").length) {
          trigger = $("#header").outerHeight(true) - 40; //small header
        } else {
          trigger = $("#header").outerHeight(true); //normal header
        }

        if ($("body.user-logged-in").length) {
          trigger += $("#toolbar-bar").outerHeight(true); //add drupal toolbar to trigger
        }

        $(window).scroll(function () {
          if (window.matchMedia('(min-width:768px)').matches) {
            var blogHeight = $blogMenu.outerHeight();
            var newOffset = $("#header").offset().top + $("#header").outerHeight(true);
            var stopTrigger = $(".site-footer").offset().top - blogHeight - 30; //point above footer where affix should stop

            if (newOffset >= stopTrigger) {
              newOffset = stopTrigger;
            }
            
            if (offset - $(window).scrollTop() <= trigger) {
              $blogMenu.offset({ top: newOffset + 15 });
            } else {
              $blogMenu.css("top", "");
            }
          }
 ...
 */
(function($, Drupal) {
  Drupal.behaviors.editorialBootstrapWebmasters = {
    attach: function(context, settings) {
      if ($('aside.pipa-workflow').length) {
        $('aside.pipa-workflow').hover(function() {
          $('aside.pipa-workflow ul').toggle();
        });

        $('aside.pipa-workflow h3').click(function() {
          $('aside.pipa-workflow ul').toggle();
        });
      }
    }
...
(function ($, Drupal) {
  Drupal.behaviors.editorial_content_teasers = {
    attach: function (context, settings) {
      if (context !== document) return;

      $(".do-dotting .field--name-title h4 a").dotdotdot({
        ellipsis: " \u2026 ",
        truncate: "word",
        height: 75,
        keep: null,
...
(function($, Drupal) {
  ...
  Drupal.behaviors.frontpageJustIn = {
    attach: function(context, settings) {
      if (context !== document) return;

      if ($('#just-in').length) {
        // @todo: fix height Just In block
        //console.log('Just in block');
      }
    }
  };
})(jQuery, Drupal);;
(function($, Drupal) {
  ...
  Drupal.behaviors.frontpageJustIn = {
    attach: function(context, settings) {
      ...
      if ($('.frontpage-block-region').length) {
        $('.frontpage-block-region .block-item a').click(function(event) {
          // if clicked on a link in the block, stop click event from bubbling to next function
          event.stopPropagation();
        });

        $('.frontpage-block-region .block-item').click(function() {
...
;
(function ($, Drupal) {
  Drupal.behaviors.editorialBaseLanguageSwitcher = {
    attach: function (context, settings) {
      if (context !== document) return;
